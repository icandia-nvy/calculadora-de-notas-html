    ...
    <div ref={dropdownRef} className="absolute bg-white rounded-md shadow-lg z-[9999] p-4 border w-64" style={{ top: `${position.top}px`, left: `${position.left}px` }}>
      <div className="space-y-3">
        <div className="flex items-center">
          <input type="checkbox" checked={evalData.differentiatedScores.enabled} onChange={() => setEvalData(prev => ({ ...prev, differentiatedScores: { ...prev.differentiatedScores, enabled: !prev.differentiatedScores.enabled } }))} className="h-4 w-4 text-blue-600 border-gray-300 rounded"/>
          ...
          <div className="space-y-2">
            {['green', 'yellow', 'blue'].map(color => (
              <div key={color}>
                ...
                <input type="text" value={evalData.differentiatedScores[color]} onChange={e => {
                  const normalized = normalizeNumberInput(e.target.value);
                  if (normalized !== null) setEvalData(prev => ({ ...prev, differentiatedScores: { ...prev.differentiatedScores, [color]: normalized } }));
    ...
    <header className="flex flex-wrap justify-between items-center mb-6 pb-4 border-b-2 border-gray-200">
      <div className="flex items-center gap-2">
        ...
        <span className={`ml-2 text-sm ${isSaving ? 'text-yellow-600 animate-pulse' : 'text-green-600'}`}>
          {isSaving ? 'Guardando…' : `✔ Guardado${lastSaved ? ' ' + new Date(lastSaved).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}) : ''}`}
        </span>
      </div>
      <div className="flex items-center space-x-2 mt-4 sm:mt-0">
        <div className="relative">
          ...
          {isAddEvalDropdownOpen && (
            <div className="absolute z-50">
              <AddEvalDropdown onAdd={onAddEvaluation} onClose={() => setAddEvalDropdownOpen(false)} />
            </div>
          )}
        </div>
...
const AddEvalDropdown = React.memo(({ onAdd, onClose }) => {
  ...
  return (
    <div ref={dropdownRef} className="mt-2 w-64 bg-white rounded-md shadow-lg z-30 p-4 border">
      <div className="space-y-3">
        <div>
...
const StudentRow = React.memo(({
  ...
  return (
    <tr className={`group h-14 ${student.highlight ? `highlight-${student.highlight}`:''} ${isActive ? 'ring-2 ring-blue-400 bg-blue-50/40' : ''}`}>
      {evaluations.map((ev, e) => (
        <React.Fragment key={ev.id}>
          <td className={`p-1 text-sm text-center ${!student.highlight && e % 2 !== 0 ? 'bg-gray-50' : ''}`}>
            <input type="text" value={student.scores[e] ?? ''}
...
const GradesTable = ({ sheet, onUpdateSheet, globalSettings }) => {
  ...
  const handleScoreChange = React.useCallback((r, e, v) => {
    ...
    onUpdateSheet({ ...sheet, studentData: d });
  }, [sheet, onUpdateSheet]);
  ...
  const StudentColumnPortal = studentRoot && (
    <table className="table-fixed border-separate" style={{ borderSpacing: 0 }}>
      ...
      {sheet.studentData.map((s, r) => (
        <tr key={s.id} className={`group h-14 ${s.highlight ? `highlight-${s.highlight}`:''} ${editingRow === r ? 'bg-blue-50/40' : ''}`}>
          ...
          <td className="p-1 text-sm">
            <div className="flex items-center">
              <input type="text" value={s.name} onChange={e => handleNameChange(r, e.target.value)} className={`bg-transparent w-full focus:bg-yellow-100 p-2 rounded ${editingRow === r ? 'font-bold text-blue-700' : ''}`}/>
  ...
  const GradesColumnPortal = gradesRoot && (
    <table className="min-w-full border-separate" style={{ borderSpacing: 0 }}>
      ...
      {sheet.studentData.map((s, r) => (
        <StudentRow key={s.id} student={s} rowIndex={r} evaluations={sheet.evaluations}
                    ...
                    handleScoreBlur={handleScoreBlur} handleKeyDown={handleKeyDown}
                    inputRefs={inputRefs} setEditingRow={setEditingRow} isActive={editingRow === r}/>
      ))}
      </tbody>
    </table>
  );
  ...
  return (
    <div className="bg-white p-2 sm:p-4 rounded-b-lg shadow-md flex">
      <div ref={setStudentRoot} className="relative flex-shrink-0">{StudentColumnPortal}</div>
      <div ref={setGradesRoot} className="overflow-x-auto flex-grow">{GradesColumnPortal}</div>
    </div>
  );
};
...
const App = () => {
  ...
  React.useEffect(() => {
    const onKey = (e) => {
      if (e.ctrlKey && e.key === 'Enter') { e.preventDefault(); if (activeSheet) { const newStudent = { id: generateId(), name: `Estudiante ${activeSheet.studentData.length + 1}`, scores: Array(activeSheet.evaluations.length).fill(''), highlight: null }; handleUpdateSheet({ ...activeSheet, studentData: [...activeSheet.studentData, newStudent] }); } }
      if (e.ctrlKey && e.shiftKey && e.key === 'Enter') { e.preventDefault(); if (activeSheet) { const ne = { name: 'Nueva Evaluación', maxScore: 10, exigency: 60 }; const newEvaluation = { ...ne, id: generateId(), differentiatedScores: { enabled: false, green: ne.maxScore, yellow: ne.maxScore, blue: ne.maxScore } }; const updatedSheet = { ...activeSheet, evaluations: [...activeSheet.evaluations, newEvaluation], studentData: activeSheet.studentData.map(s => ({ ...s, scores: [...s.scores, ''] })) }; handleUpdateSheet(updatedSheet); } }
    };
      ...
      {sheets.length > 0 && activeSheetId && activeSheet ? (
        <div>
          <Tabs
            ...
            onRenameTab={(id, name) => setSheets(prev => prev.map(s => s.id === id ? { ...s, name } : s))}
            onAddTab={() => {
              const name = `Hoja ${sheets.length + 1}`;
              const newSheet = { id: generateId(), name, studentHeader: 'Alumnos', evaluations: [], studentData: [], studentColumnWidth: 200 };
              setSheets(p => [...p, newSheet]);
            ...
            onReorderSheets={(dragIndex, dropIndex) => {
              setSheets(prevSheets => {
                const newSheets = [...prevSheets];
                const [draggedItem] = newSheets.splice(dragIndex, 1);
                newSheets.splice(dropIndex, 0, draggedItem);
